...
### Fuel Connector - Provider instances

When passing a provider instance as a `Promise<Provider>` to the connectors, each connector instance will create it's own provider instance. Leading to multiple chain requests and multiple provider instances.

This has been resolved now as we have removed the async `Provider` instantiation, as of [#3514](https://github.com/FuelLabs/fuels-ts/pull/3514). I believe we should follow up with the `@fuels/connectors` related packages and remove all types for `Promise<Provider>` and just use the `Provider` class directly.

We should always require the passing of the Provider instance, to ensure that the same provider instance is used across all connectors.
...
There are also numerous instances whereby we instantiate a new Provider instance, just to get the chain ID (for [example](https://github.com/FuelLabs/fuel-connectors/blob/b21855feaf6275b16f626156ba7d1c74262f3a5b/packages/react/src/hooks/useIsSupportedNetwork.tsx#L50)). This leads to multiple provider instances and missing of caches between these provider instances.

### Fuel Provider (unexpected chain refetch)

> [!NOTE]
> This could be related to the [above](#fuel-connector-provider-instances).

@TODO: Check that the refetch chain config is working as expected.

- Assuming we have the chain cached.

- We make a request against the getConsensusParametersVersion.

```graphql
{"query":"query getConsensusParametersVersion {\n  chain {\n    latestBlock {\n      header {\n        consensusParametersVersion\n      }\n    }\n  }\n}","operationName":"getConsensusParametersVersion"}
```

```JSON
{"data":{"chain":{"latestBlock":{"header":{"consensusParametersVersion":"4"}}}}}
```

- Which is the same as the cached chain config.

- But we are still refetching the chain config????
...
- Evidence that the inverted submission flow would improve the execution for both the liquidity + swap flows.
- To avoid re-requesting the chain config, we could cache the chain config in the users local storage.
  - I'd be good if we prepared a guide for "Production grade Fuel" to help outline the best practices.
