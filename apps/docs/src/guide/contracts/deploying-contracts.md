...
In the above example, we also pass a `chunkSizeOverride` option to the deployment method. The SDK will attempt to chunk the contract to the most optimal amount, however, the transaction size can fluctuate and you can also be limited by request size limits against the node. By default, we set a value for `chunkSizeOverride` of 0.05 (5%), however you can adjust this to suit your needs and ensure the transaction passes. It must be set to a value between 0 and 1.
