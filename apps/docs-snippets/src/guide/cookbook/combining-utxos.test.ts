 ...
 * @group browser
 */
describe('Combining UTXOs', () => {
  it('Combines UTXOs', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      const initTx = await fundingWallet.transfer(wallet.address, 1000, provider.getBaseAssetId());
      await initTx.waitForResult();
    }

    // We can fetch the coins to see how many UTXOs we have, and confirm it is 10.
    const { coins: initialCoins } = await wallet.getCoins(provider.getBaseAssetId());
    ...
    const combineTx = await wallet.transfer(wallet.address, 9500, provider.getBaseAssetId());
    ...
    const { coins: combinedCoins } = await wallet.getCoins(provider.getBaseAssetId());
    ...
    expect(initialCoins.length).toBe(10);
    expect(initialBalance.toNumber()).toBe(10_000);
    expect(combinedCoins.length).toBe(2);
    expect(combinedBalance.toNumber()).toBe(9_998);
  });
});
