...
describe(__filename, () => {
  ...
  beforeAll(async () => {
    contract = await createAndDeployContractFromProject(
      DocSnippetProjectsEnum.INPUT_OUTPUT_TYPES,
    );
  });
  ...
  it('should successfully call a function with an Address type input and output parameters', async () => {
    ...
    const callResponse = await contract.functions
  ...
  it('should successfully call a function with a Identity type input and output parameters', async () => {
    ...
    const callResponse1 = await contract.functions
      .identity(addressIdentityInput)
      .simulate();
    ...
    const addressFromOutput: Address = Address.fromB256(
      addressStringFromOutput,
    );
    ...
    const callResponse2 = await contract.functions
