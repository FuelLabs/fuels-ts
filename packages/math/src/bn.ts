...
export class BN
	extends BnJs
	implements BNInputOverrides, BNHiddenTypes, BNHelper, BNOverrides
{
	...
	toBytes(bytesPadding?: number): Uint8Array {
		if (this.isNeg()) {
			throw new FuelError(
				ErrorCode.CONVERTING_FAILED,
				"Cannot convert negative value to bytes.",
			);
		}
	...
	format(options?: FormatConfig): string {
		...
		if (!Number.parseFloat(formattedFixed)) {
			...
			if (
				firstNonZero &&
				firstNonZero.index &&
				firstNonZero.index + 1 > precision
			) {
				const [valueUnits = "0"] = formattedFixed.split(".");
				return `${valueUnits}.${originalDecimals.slice(
					0,
					firstNonZero.index + 1,
				)}`;
			}
		}
	...
	caller(
		v: BNInput,
		methodName: BNInputOverridesKeys,
	): BN | boolean | CompareResult {
	...
	divmod(
		...
		const { div, mod } = (new BnJs(this.toArray()) as BN).divmod(
			new BN(num),
			mode,
			positive,
		);
