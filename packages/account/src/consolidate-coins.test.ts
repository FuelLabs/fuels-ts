...
describe('consolidate-coins', () => {
  ...
  describe('Base asset', () => {
    it('should consolidate base asset just fine [ACCOUNT HAS LESS THAN MAX INPUTS]', async () => {
      const maxInputs = 255;
    ...
    it('should consolidate base asset just fine [ACCOUNT HAS EXACTLY MAX INPUTS]', async () => {
      const maxInputs = 255;
    ...
    it('should NOT attempt to consolidate just one base asset coin', async () => {
      const maxInputs = 255;
  ...
  describe('Non-base asset', () => {
    ...
    it('should NOT attempt to consolidate just one asset coin', async () => {
      const maxInputs = 255;
      const totalCoins = maxInputs + 1; // Only one remaining coin for the second consolidation tx
      const { provider, wallets } = await setupTest({ maxInputs, coinsPerAsset: totalCoins });
      const [wallet] = wallets;
