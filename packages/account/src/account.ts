...
export class Account extends AbstractAccount {
  ...
  async getResourcesToSpend(
    ...
    return this.provider.getResourcesToSpend(
      this.address,
      quantities,
      excludedIds,
    );
  ...
  async fund<T extends TransactionRequest>(request: T, params: EstimatedTxParams): Promise<T> {
    ...
    request.inputs.filter(isRequestInputResource).forEach((input) => {
      ...
      if (quantitiesDict[assetId]) {
        quantitiesDict[assetId].owned = quantitiesDict[assetId].owned.add(
          input.amount,
        );
      }
    });
  ...
  async transfer(
    ...
    const assetIdToTransfer = assetId ?? this.provider.getBaseAssetId();
    const request = await this.createTransfer(
      destination,
      amount,
      assetIdToTransfer,
      txParams,
  ...
  async withdrawToBaseLayer(
    ...
    const recipientDataArray = arrayify(
      '0x'.concat(
        recipientAddress.toHexString().substring(2).padStart(64, '0'),
      ),
    );
  ...
  async sendTransaction(
    ...
    if (this._connector) {
      return this.provider.getTransactionResponse(
        await this._connector.sendTransaction(
          this.address.toString(),
          transactionRequestLike,
        ),
      );
