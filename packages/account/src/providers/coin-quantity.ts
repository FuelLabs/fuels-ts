...
/** @hidden */
export const coinQuantityfy = (
	coinQuantityLike: CoinQuantityLike,
): CoinQuantity => {
	let assetId;
	let amount;
	let max;
...
export const addAmountToCoinQuantities = (
	...
	const assetIdx = coinQuantities.findIndex(
		(coinQuantity) => coinQuantity.assetId === assetId,
	);

	if (assetIdx !== -1) {
		coinQuantities[assetIdx].amount =
			coinQuantities[assetIdx].amount.add(amount);
	} else {
