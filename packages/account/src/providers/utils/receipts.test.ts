...
describe('assembleReceiptByType', () => {
  it('should return a ReceiptCall receipt when GqlReceiptType.Call is provided', () => {
    ...
    expect(receipt.amount).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.amount),
    );
    ...
    expect(receipt.param1).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.param1),
    );
    expect(receipt.param2).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.param2),
    );
  ...
  it('should return a ReceiptPanic receipt when GqlReceiptType.Panic is provided', () => {
    ...
    expect(receipt.is).toStrictEqual(new BN(MOCK_GQL_RECEIPT_FRAGMENT.is));
    expect(receipt.pc).toStrictEqual(new BN(MOCK_GQL_RECEIPT_FRAGMENT.pc));
    expect(receipt.reason).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.reason),
    );
  });
  ...
  it('should return a ReceiptTransfer receipt when GqlReceiptType.Transfer to address is provided', () => {
    ...
    expect(receipt.from).toStrictEqual(
      MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.contractId,
    );
    expect(receipt.to).toStrictEqual(
      MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.toAddress,
    );
    expect(receipt.assetId).toStrictEqual(
      MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.assetId,
    );
    expect(receipt.is).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.is),
    );
    expect(receipt.pc).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.pc),
    );
    expect(receipt.amount).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT_TO_ADDRESS.amount),
    );
  });
  ...
  it('should return a ReceiptScriptResult when GqlReceiptType.ScriptResult is provided', () => {
    ...
    expect(receipt.result).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.result),
    );
    expect(receipt.gasUsed).toStrictEqual(
      new BN(MOCK_GQL_RECEIPT_FRAGMENT.gasUsed),
    );
  });
