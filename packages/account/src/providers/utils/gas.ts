...
import { type Input, ReceiptType } from '@fuel-ts/transactions';
...
/** @hidden */
export const getGasUsedFromReceipts = (
  ...
  const gasUsed = scriptResult.reduce(
...
export function resolveGasDependentCosts(
  byteSize: BNInput,
  ...
  if ('unitsPerGas' in gasDependentCost) {
    dependentValue = bn(byteSize).div(
      bn((<GqlLightOperation>gasDependentCost).unitsPerGas),
    );
  } else {
    dependentValue = bn(byteSize).mul(
      bn((<GqlHeavyOperation>gasDependentCost).gasPerUnit),
    );
  }
...
export function gasUsedByInputs(
  ...
  const vmInitializationCost = resolveGasDependentCosts(
    txBytesSize,
...
export function getMinGas(params: IGetMinGasParams) {
  ...
  const vmInitGas = resolveGasDependentCosts(
    txBytesSize,
    gasCosts.vmInitialization,
  );
...
export function getMaxGas(params: IGetMaxGasParams) {
  ...
  if (witnessLimit?.gt(0) && witnessLimit.gte(witnessesLength)) {
    remainingAllowedWitnessGas = bn(witnessLimit)
      .sub(witnessesLength)
      .mul(gasPerByte);
  }
...
export function calculateMetadataGasForTxCreate({
  ...
  const contractRootGas = resolveGasDependentCosts(
    contractBytesSize,
    gasCosts.contractRoot,
  );
  const stateRootGas = resolveGasDependentCosts(
    stateRootSize,
    gasCosts.stateRoot,
  );
  ...
  const contractIdGas = resolveGasDependentCosts(
    contractIdInputSize,
    gasCosts.s256,
  );
  const metadataGas = contractRootGas
    .add(stateRootGas)
    .add(txIdGas)
    .add(contractIdGas);
