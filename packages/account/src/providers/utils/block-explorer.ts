...
export const buildBlockExplorerUrl = (
  ...
  const { blockExplorerUrl, path, providerUrl, address, txId, blockNumber } =
    options;
  ...
  const protocol = cleanBlockExplorerUrl.match(/^https?:\/\//)
    ? ''
    : 'https://';
  const providerUrlProtocol = cleanProviderUrl?.match(/^https?:\/\//)
    ? ''
    : 'https://';

  const url = `${protocol}${cleanBlockExplorerUrl}/${cleanPath}${
    encodedProviderUrl
      ? `?providerUrl=${providerUrlProtocol}${encodedProviderUrl}`
      : ''
  }`;
