...
describe('Provider', () => {
  ...
  it('can get all chain info', async () => {
    ...
    expect(
      consensusParameters.txParameters.maxBytecodeSubsections,
    ).toBeDefined();
    ...
    expect(
      consensusParameters.scriptParameters.maxScriptDataLength,
    ).toBeDefined();

    expect(consensusParameters.contractParameters.version).toBeDefined();
    expect(
      consensusParameters.contractParameters.contractMaxSize,
    ).toBeDefined();
    expect(
      consensusParameters.contractParameters.maxStorageSlots,
    ).toBeDefined();

    expect(consensusParameters.predicateParameters.version).toBeDefined();
    expect(
      consensusParameters.predicateParameters.maxPredicateLength,
    ).toBeDefined();
    expect(
      consensusParameters.predicateParameters.maxPredicateDataLength,
    ).toBeDefined();
    expect(
      consensusParameters.predicateParameters.maxGasPerPredicate,
    ).toBeDefined();
    expect(
      consensusParameters.predicateParameters.maxMessageDataLength,
    ).toBeDefined();
  });
  ...
  it('can change the provider url of the current instance', async () => {
    ...
    const provider = await Provider.create(providerUrl1, {
      fetch: (url: string, options?: RequestInit) =>
        getCustomFetch('getVersion', { nodeInfo: { nodeVersion: url } })(
          url,
          options,
        ),
    });
  ...
  it('can accept a custom fetch function', async () => {
    ...
    const provider = await Provider.create(providerUrl, {
      fetch: getCustomFetch('getVersion', {
        nodeInfo: { nodeVersion: '0.30.0' },
      }),
    });
  ...
  it('can force-produce blocks', async () => {
    ...
    const producedBlockHeigh = await provider.produceBlocks(
      amountOfBlocksToProduce,
    );

    const producedBlock = await provider.getBlock(
      producedBlockHeigh.toNumber(),
    );

    expect(producedBlock).toBeDefined();

    const oldest: Date = DateTime.fromTai64(timeLastBlockProduced);
    const newest: Date = DateTime.fromTai64(
      producedBlock?.time || DateTime.TAI64_NULL,
    );
  ...
  it.skip('can force-produce blocks with custom timestamps', async () => {
    ...
    const startTime =
      new Date(latestBlockUnixTimestampBeforeProduce).getTime() + 1000; // 1s after the latest block
    ...
    const expectedBlocks = Array.from(
      ...
      (_, i) => ({
        ...
        time: DateTime.fromUnixMilliseconds(
          startTime + i * blockTimeInterval,
        ).toTai64(),
      }),
    );
  ...
  it('can cacheUtxo [invalid numerical]', async () => {
    const { error } = await safeExec(() =>
      Provider.create(FUEL_NETWORK_URL, { cacheUtxo: -500 }),
    );
  ...
  it('can getMessageProof with all data', async () => {
    ...
    const provider = await Provider.create(FUEL_NETWORK_URL, {
      fetch: async (url, options) =>
        getCustomFetch('getMessageProof', {
          messageProof: MESSAGE_PROOF_RAW_RESPONSE,
        })(url, options),
    });
  ...
  it('can getMessageStatus', async () => {
    ...
    const provider = await Provider.create(FUEL_NETWORK_URL, {
      fetch: async (url, options) =>
        getCustomFetch('getMessageStatus', {
  ...
  it('should ensure getChain and getNode uses the cache and does not fetch new data', async () => {
    ...
    const spyFetchChainAndNodeInfo = vi.spyOn(
      Provider.prototype,
  ...
  it('warns on difference between major client version and supported major version', async () => {
    ...
    const majorMismatch = major === '0' ? 1 : Number.parseInt(patch, 10) - 1;
...
Supported fuel-core version: ${mock.supportedVersion}.`,
  ...
  it('warns on difference between minor client version and supported minor version', async () => {
    const { FUEL_CORE } = versions;
    const [major, minor, patch] = FUEL_CORE.split('.');
    const minorMismatch = minor === '0' ? 1 : Number.parseInt(patch, 10) - 1;
...
Supported fuel-core version: ${mock.supportedVersion}.`,
  ...
  it.skip('should ensure estimated fee values on getTransactionCost are never 0', async () => {
    ...
    const calculateGasFeeMock = vi
      .spyOn(gasMod, 'calculateGasFee')
  ...
  it('subscriptions: does not throw when stream contains more than one "data:"', async () => {
    ...
    for await (const { submitAndAwait } of provider.operations.submitAndAwait({
      ...
      expect((<SubmittedStatus>submitAndAwait).time).toEqual(
        'data: 4611686020137152060',
      );
    }
  });
  ...
  test('subscriptions: ignores keep-alive messages', async () => {
    ...
    fetchSpy.mockImplementationOnce(() =>
      Promise.resolve(new Response(readableStream)),
    );
  ...
  it('subscriptions: throws if the stream data string parsing fails for some reason', async () => {
    ...
    await expectToThrowFuelError(
      async () => {
        for await (const {
          submitAndAwait,
        } of provider.operations.submitAndAwait({
          encodedTransaction: "it's mocked so doesn't matter",
        })) {
  ...
  test('requestMiddleware modifies the request before being sent to the node [sync]', async () => {
    ...
    await Provider.create(FUEL_NETWORK_URL, {
      requestMiddleware: (request) => {
        request.headers ??= {};
        (request.headers as Record<string, string>)['x-custom-header'] =
          'custom-value';
        return request;
      },
    });
  ...
  test('requestMiddleware works for subscriptions', async () => {
    ...
    const subscriptionCall = fetchSpy.mock.calls.find(
      (call) => call[0] === `${provider.url}-sub`,
    );
