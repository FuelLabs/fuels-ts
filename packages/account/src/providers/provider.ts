...
export default class Provider {
	...
	protected constructor(
		...
		this.cache = options.cacheUtxo
			? new MemoryCache(options.cacheUtxo)
			: undefined;
	}
...
Supported fuel-core version: ${supportedVersion}.`,
			...
			if (isSubscription) {
				return new FuelGraphqlSubscriber({
					...
					fetchFn: (url, requestInit) =>
						fetchFn(url as string, requestInit, this.options),
					variables: vars,
				});
			}
	...
	async sendTransaction(
		...
		if (awaitExecution) {
			const subscription = this.operations.submitAndAwait({
				encodedTransaction,
			});
		...
		if (inputs) {
			inputs.forEach((input, index) => {
				if ("predicateGasUsed" in input && bn(input.predicateGasUsed).gt(0)) {
					(<CoinTransactionRequestInput>(
						transactionRequest.inputs[index]
	...
	async estimateTxDependencies(
		...
		for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
			...
			const hasMissingOutputs =
				missingOutputVariables.length !== 0 ||
				missingOutputContractIds.length !== 0;
	...
	async estimateMultipleTxDependencies(
		...
		while (transactionsToProcess.length > 0 && attempt < MAX_RETRIES) {
			...
			for (let i = 0; i < dryRunResults.dryRun.length; i++) {
				...
				const { missingOutputVariables, missingOutputContractIds } =
					getReceiptsWithMissingData(result.receipts);
				const hasMissingOutputs =
					missingOutputVariables.length > 0 ||
					missingOutputContractIds.length > 0;
				...
				if (hasMissingOutputs && request?.type === TransactionType.Script) {
					...
					serializedTransactionsMap.set(
						requestIdx,
						hexlify(request.toTransactionBytes()),
					);
	...
	async getTransactionCost(
		...
		txRequestClone.fundWithFakeUtxos(
			allQuantities,
			baseAssetId,
		...
		if (
			resourcesOwner &&
			"populateTransactionPredicateData" in resourcesOwner
		) {
			(resourcesOwner as Predicate<[]>).populateTransactionPredicateData(
				txRequestClone,
			);
		}
		...
		let { maxFee, maxGas, minFee, minGas, gasPrice, gasLimit } =
			await this.estimateTxGasAndFee({
		...
		if (isScriptTransaction) {
			...
			gasUsed = isScriptTransaction
				? getGasUsedFromReceipts(receipts)
				: gasUsed;

			txRequestClone.gasLimit = gasUsed;

			({ maxFee, maxGas, minFee, minGas, gasPrice } =
				await this.estimateTxGasAndFee({
	...
	async getCoins(
		...
		const result = await this.operations.getCoins({
			...
			filter: {
				...
				assetId: assetId && hexlify(assetId),
			},
		});
	...
	async getResourcesToSpend(
		...
		if (this.cache) {
			const uniqueUtxos = new Set(
				excludeInput.utxos.concat(
					this.cache?.getActiveData().map((id) => hexlify(id)),
				),
			);
	...
	async getTransaction<TTransactionType = void>(
		...
		const { transaction } = await this.operations.getTransaction({
			transactionId,
		});
		if (!transaction) {
			return null;
		...
		return {
			...
			messageBlockHeader: {
				...
				consensusParametersVersion: Number(
					messageBlockHeader.consensusParametersVersion,
				),
				eventInboxRoot: messageBlockHeader.eventInboxRoot,
				stateTransitionBytecodeVersion: Number(
					messageBlockHeader.stateTransitionBytecodeVersion,
				),
			},
			commitBlockHeader: {
				...
				stateTransitionBytecodeVersion: Number(
					commitBlockHeader.stateTransitionBytecodeVersion,
				),
			},
	...
	async produceBlocks(amount: number, startTime?: number) {
		const { produceBlocks: latestBlockHeight } =
			await this.operations.produceBlocks({
	...
	async getRelayedTransactionStatus(
		...
		const { relayedTransactionStatus } =
			await this.operations.getRelayedTransactionStatus({
