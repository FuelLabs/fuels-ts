...
describe('operations', () => {
  ...
  describe('getPayProducerOperations', () => {
    ...
    it('should ensure getPayProducerOperations return empty', () => {
      const res1 = getPayProducerOperations([
        MOCK_OUTPUT_CONTRACT_CREATED,
        MOCK_OUTPUT_CHANGE,
      ]);
  ...
  describe('addOperation', () => {
    ...
    it('should just add operation when its the first one', () => {
      const operations = addOperation([], operationContractCall);
      expect(operations.length).toEqual(1);
      expect(operations[0]).toStrictEqual(operationContractCall);
    });

    it('should not stack operations with different name / from / to', () => {
      const baseOperations = addOperation([], operationContractCall);
      const operationsDifName = addOperation(baseOperations, {
        ...operationContractCall,
        name: OperationName.contractCreated,
      });
    ...
    it('should return prev operation if no asset is sent to add', () => {
      ...
      expect(
        JSON.parse(JSON.stringify(operationsAddedSameAsset[0].assetsSent)),
      ).toEqual(JSON.parse(JSON.stringify(baseOperations[0].assetsSent)));
    });
    ...
    it('should stack when same asset is added', () => {
      ...
      ).toEqual(operationContractCall.assetsSent[0].amount.mul(2).valueOf());
      expect(operationsAddedSameAsset[0].assetsSent?.[0]?.assetId).toEqual(
        operationContractCall.assetsSent[0].assetId,
      );
    });

    it('should stack when same asset is added together with a different asset [CONTRACT-CALL]', () => {
      const difAssetId = '0x0012300000000000000000000000000000000001';
      const operationTwoAssets: Operation = {
        ...operationContractCall,
        assetsSent: [
          {
            ...operationContractCall.assetsSent[0],
          },
          {
            ...operationContractCall.assetsSent[0],
            assetId: difAssetId,
      ...
      ).toEqual(operationContractCall.assetsSent[0].amount.valueOf());
  ...
  describe('isType', () => {
    it('should isType return by type', () => {
      expect(
        isType(TransactionType.Create, TransactionTypeName.Create),
      ).toBeTruthy();
      expect(
        isType(TransactionType.Script, TransactionTypeName.Script),
      ).toBeTruthy();
      expect(
        isType(TransactionType.Mint, TransactionTypeName.Mint),
      ).toBeTruthy();
      expect(
        isType(TransactionType.Script, TransactionTypeName.Create),
      ).toBeFalsy();
      expect(
        isType(TransactionType.Mint, TransactionTypeName.Script),
      ).toBeFalsy();
      expect(
        isType(TransactionType.Create, TransactionTypeName.Mint),
      ).toBeFalsy();
    });
  ...
  describe('getReceipts', () => {
    ...
    it('should getReceiptsMessageOut return correct receipts', () => {
      const expected: TransactionResultMessageOutReceipt = {
        ...
        nonce:
          '0x66c4d70c08ff30cd2d9dae0b6fd05972997579328529bb0605dd604afedfdf93',
        recipient:
          '0x000000000000000000000000f39fd6e51aad88f6f4ce6ab8827279cfffb92266',
        sender:
  ...
  it('should ensure getTransactionTypeName works as expected', () => {
    expect(getTransactionTypeName(TransactionType.Create)).toBe(
      TransactionTypeName.Create,
    );
    expect(getTransactionTypeName(TransactionType.Mint)).toBe(
      TransactionTypeName.Mint,
    );
    expect(getTransactionTypeName(TransactionType.Script)).toBe(
      TransactionTypeName.Script,
    );

    expect(() =>
      getTransactionTypeName('' as unknown as TransactionType),
