...
export const calculateTXFeeForSummary = (
	...
	if (type === TransactionType.Create) {
		const { bytecodeWitnessIndex, storageSlots } =
			transaction as TransactionCreate;

		const contractBytesSize = bn(
			arrayify(witnesses[bytecodeWitnessIndex].data).length,
		);
	...
	const witnessLimit = policies.find(
		(policy) => policy.type === PolicyType.WitnessLimit,
	)?.data as BN | undefined;

	const witnessesLength = witnesses.reduce(
