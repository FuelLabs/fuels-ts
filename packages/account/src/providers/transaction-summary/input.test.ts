...
describe('transaction-summary/input', () => {
  it('should ensure getInputsCoin return correct inputs', () => {
    const coinInputs = getInputsCoin([
      MOCK_INPUT_COIN,
      MOCK_INPUT_CONTRACT,
      MOCK_INPUT_MESSAGE,
    ]);
    ...
    const emptyInputs = getInputsCoin([
      MOCK_INPUT_CONTRACT,
      MOCK_INPUT_MESSAGE,
    ]);
  ...
  it('should ensure getInputsMessage return correct inputs', () => {
    ...
    const emptyInputs = getInputsMessage([
      MOCK_INPUT_CONTRACT,
      MOCK_INPUT_COIN,
    ]);
    expect(emptyInputs.length).toEqual(0);
  ...
  it('should ensure getInputsContract return correct inputs', () => {
    ...
    const emptyInputs = getInputsContract([
      MOCK_INPUT_COIN,
      MOCK_INPUT_MESSAGE,
    ]);
    expect(emptyInputs.length).toEqual(0);
  ...
  it('should ensure getInputContractFromIndex returns undefined if index is null', () => {
    const res = getInputContractFromIndex(
      [MOCK_INPUT_COIN],
  ...
  it('should ensure getInputAccountAddress return correct address of owner of input', () => {
    expect(getInputAccountAddress(MOCK_INPUT_COIN)).toEqual(
      MOCK_INPUT_COIN.owner,
    );

    expect(getInputAccountAddress(MOCK_INPUT_MESSAGE)).toEqual(
      MOCK_INPUT_MESSAGE.recipient,
    );
  ...
  it('should ensure getInputFromAssetId return correct input to pay for that assetId', () => {
    ...
    expect(
      getInputFromAssetId([inputCoin1, inputCoin2], ZeroBytes32),
    ).toStrictEqual(inputCoin1);
    expect(
      getInputFromAssetId([inputCoin1, inputCoin2], ASSET_A),
    ).toStrictEqual(inputCoin2);

    expect(
      getInputFromAssetId([MOCK_INPUT_MESSAGE], ZeroBytes32),
    ).toStrictEqual(MOCK_INPUT_MESSAGE);
  });
});
