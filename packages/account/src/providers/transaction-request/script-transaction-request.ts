 ...
 * @hidden
 */
export interface ScriptTransactionRequestLike
  extends BaseTransactionRequestLike {
...
export class ScriptTransactionRequest extends BaseTransactionRequest {
  ...
  constructor({
    ...
    this.scriptData = arrayify(
      scriptData ?? returnZeroScript.encodeScriptData(),
    );
    this.abis = rest.abis;
  }
  ...
  getContractInputs(): ContractTransactionRequestInput[] {
    return this.inputs.filter(
      (input): input is ContractTransactionRequestInput =>
  ...
  getContractOutputs(): ContractTransactionRequestOutput[] {
    return this.outputs.filter(
      (output): output is ContractTransactionRequestOutput =>
        output.type === OutputType.Contract,
    );
  }
  ...
  getVariableOutputs(): VariableTransactionRequestOutput[] {
    return this.outputs.filter(
      (output): output is VariableTransactionRequestOutput =>
        output.type === OutputType.Variable,
    );
  }
  ...
  addContractInputAndOutput(
    ...
    if (
      this.getContractInputs().find(
        (i) => i.contractId === contractAddress.toB256(),
      )
    ) {
      return this;
    }
