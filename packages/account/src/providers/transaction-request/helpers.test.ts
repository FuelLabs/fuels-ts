...
import {
  generateFakeCoin,
  generateFakeMessageCoin,
} from '../../test-utils/resources';
import {
...
describe('helpers', () => {
  ...
  describe('getAssetAmountInRequestInputs', () => {
    ...
    it('should sum assets on TransactionRequestsInputs just fine', () => {
      ...
      const tx = new ScriptTransactionRequest();
      tx.addResources([
        generateFakeCoin({ assetId: assetA, amount }),
        generateFakeCoin({ assetId: assetA, amount }),
        generateFakeCoin({ assetId: assetB, amount }),
      ]);
  ...
  describe('cacheRequestInputsResources', () => {
    ...
    it('should sum assets on TransactionRequestsInputs just fine', () => {
      const amount = bn(100);
      const input1 = generateFakeRequestInputCoin({ assetId: assetA, amount });
      const input2 = generateFakeRequestInputCoin({ assetId: assetB, amount });
