...
export abstract class BaseTransactionRequest
  ...
  protected getBaseTransaction(): ToBaseTransactionResponse {
    ...
    const { policyTypes, policies } =
      BaseTransactionRequest.getPolicyMeta(this);
  ...
  async addAccountWitnesses(account: Account | Account[]) {
    ...
    await Promise.all(
      accounts.map(async (acc) => {
        this.addWitness(
          await acc.signTransaction(<TransactionRequestLike>this),
        );
      }),
    );
  ...
  getCoinOutputs(): CoinTransactionRequestOutput[] {
    return this.outputs.filter(
      (output): output is CoinTransactionRequestOutput =>
  ...
  getChangeOutputs(): ChangeTransactionRequestOutput[] {
    return this.outputs.filter(
      (output): output is ChangeTransactionRequestOutput =>
  ...
  getCoinInputWitnessIndexByOwner(owner: AddressLike): number | undefined {
    ...
    const found = this.inputs.find((input) => {
      switch (input.type) {
        case InputType.Coin:
          return (
            hexlify((<CoinTransactionRequestInput>input).owner) ===
            ownerAddress.toB256()
          );
        case InputType.Message:
          return (
            hexlify((<MessageTransactionRequestInput>input).recipient) ===
            ownerAddress.toB256()
          );
        default:
          return false;
  ...
  getCoinOutputsQuantities(): CoinQuantity[] {
    const coinsQuantities = this.getCoinOutputs().map(
      ({ amount, assetId }) => ({
        amount: bn(amount),
      ...
      switch (i.type) {
        case InputType.Coin:
          correspondingInput = inputs.find(
            (x) => x.type === InputType.Coin && x.owner === i.owner,
          );
          break;
        case InputType.Message:
          correspondingInput = inputs.find(
