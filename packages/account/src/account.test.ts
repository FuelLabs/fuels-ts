...
import { Provider, ScriptTransactionRequest } from './providers';
...
describe('Account', () => {
  ...
  it('should execute fund just as fine', async () => {
    ...
    const addAmountToCoinQuantitiesSpy = vi.spyOn(
      providersMod,
  ...
  it('should execute sendTransaction just fine', async () => {
    ...
    const transactionRequestify = vi.spyOn(
      providersMod,
  ...
  it('can withdraw an amount of base asset', async () => {
    ...
    const messageOutReceipt = <providersMod.TransactionResultMessageOutReceipt>(
      result.receipts[0]
    );
  ...
  it('can retrieve a valid MessageProof', async () => {
    ...
    const recipientId =
      '0x00000000000000000000000047ba61eec8e5e65247d717ff236f504cf3b0a263';
    const amount = 10;
    const recipient = Address.fromB256(recipientId);
  ...
  it('can transfer amount using mutiple utxos', async () => {
    ...
    expect(receiverBalances).toEqual([
      { assetId: baseAssetId, amount: bn(110) },
    ]);
  });
