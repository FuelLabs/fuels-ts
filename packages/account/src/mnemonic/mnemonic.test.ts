...
describe('Mnemonic', () => {
  ...
  test('Word list invalid should throw', () => {
    expect(() => new Mnemonic([])).toThrowError(
      'Expected word list length of 2048, but got 0.',
    );
  });
  ...
  MnemonicSpec.forEach((spec) => {
    ...
    test(`Mnemonic to seed with pass: ${spec.entropy}`, () => {
      expect(Mnemonic.mnemonicToSeed(spec.mnemonic, spec.passphrase)).toBe(
        spec.seed,
      );
    });
    test(`Mnemonic to seed with pass: ${spec.entropy}`, () => {
