...
import type { FuelConnectorEventsType, Network, TargetObject } from './types';
...
export class Fuel extends FuelConnector {
  ...
  private async setDefaultConnector(): Promise<boolean | undefined> {
    const connectorName =
      (await this._storage?.getItem(Fuel.STORAGE_KEY)) ||
      this._connectors[0]?.name;
    if (connectorName) {
  ...
  private async callMethod(method: string, ...args: unknown[]) {
    ...
    if (
      typeof this._currentConnector[method as keyof FuelConnector] ===
      'function'
    ) {
      return (
        this._currentConnector[
          method as keyof FuelConnector
        ] as CallableFunction
      )(...args);
    }
