...
export class Signer {
  ...
  constructor(privateKey: BytesLike) {
    ...
    this.publicKey = hexlify(
      secp256k1.getPublicKey(privateKeyBytes, false).slice(1),
    );
    this.compressedPublicKey = hexlify(
  ...
  addPoint(point: BytesLike) {
    const p0 = secp256k1.ProjectivePoint.fromHex(
      arrayify(this.compressedPublicKey),
    );
  ...
  static recoverPublicKey(data: BytesLike, signature: BytesLike): string {
    ...
    const sig = new secp256k1.Signature(
      BigInt(hexlify(r)),
      BigInt(hexlify(s)),
    ).addRecoveryBit(recoveryParam);

    const publicKey = sig
      .recoverPublicKey(arrayify(data))
      .toRawBytes(false)
      .slice(1);
  ...
  static generatePrivateKey(entropy?: BytesLike) {
    return entropy
      ? hash(concat([randomBytes(32), arrayify(entropy)]))
      : randomBytes(32);
  }
