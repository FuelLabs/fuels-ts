...
describe('launchNode', () => {
  ...
  describe('without a GENESIS_SECRET', () => {
    let genesisSecret: string | undefined;

    beforeAll(() => {
      genesisSecret = process.env.GENESIS_SECRET;
      delete process.env.GENESIS_SECRET;
    });

    afterAll(() => {
      process.env.GENESIS_SECRET = genesisSecret;
    });
    ...
    test('launchNodeAndGetWallets - empty config', async () => {
      ...
      expect(process.env.GENESIS_SECRET).not.toEqual(genesisSecret);
