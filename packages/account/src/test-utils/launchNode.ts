...
export const launchNode = async ({
	...
	new Promise(async (resolve, reject) => {
		...
		const consensusKey =
			getFlagValueFromArgs(args, "--consensus-key") || defaultConsensusKey;

		const dbTypeFlagValue = getFlagValueFromArgs(args, "--db-type");
		const useInMemoryDb =
			dbTypeFlagValue === "in-memory" || dbTypeFlagValue === undefined;

		const poaInstantFlagValue = getFlagValueFromArgs(args, "--poa-instant");
		const poaInstant =
			poaInstantFlagValue === "true" || poaInstantFlagValue === undefined;
		...
		if (snapshotDir) {
			...
			fixedStateConfigJSON = fixedStateConfigJSON.replace(
				regexMakeNumber,
				"$1$2",
			);
			...
			const stateTransitionWritePath = path.join(
				tempDirPath,
				"state_transition_bytecode.wasm",
			);

			writeFileSync(
				chainConfigWritePath,
				JSON.stringify(chainConfigJson),
				"utf8",
			);
...
export const launchNodeAndGetWallets = async ({
	...
	const {
		...
		ip,
		port,
	} = await launchNode(launchNodeOptions || {});
