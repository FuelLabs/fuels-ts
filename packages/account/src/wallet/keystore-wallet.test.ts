...
describe("Keystore Wallet", () => {
	...
	const privateKey =
		"0xeac85e732b683119e62fb52ce3b04c0d2f60539cd55af34c731fcdcf802e5ef4";
	...
	it("should return a valid keystore when given correct parameters", async () => {
		// Act
		const keystore = await encryptKeystoreWallet(
			privateKey,
			address.toB256(),
			password,
		);
	...
	it("should throw an error when given an incorrect password", async () => {
		...
		const { error } = await safeExec(() =>
			decryptKeystoreWallet(keystore, wrongPassword),
		);
