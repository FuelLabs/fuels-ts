...
import {
  ...
  SCRIPT_TX_REQUEST,
  SIGNED_TX,
} from '../../test/fixtures/wallet-unlocked';
...
describe('WalletUnlocked', () => {
  const expectedPrivateKey =
    '0x5f70feeff1f229e4a95e1056e8b4d80d0b24b565674860cc213bdb07127ce1b1';
  ...
  it('Populate transaction multi-witnesses signature using wallet instance', async () => {
    ...
    const signedTransaction = await wallet.signTransaction(SCRIPT_TX_REQUEST);
    const otherSignedTransaction =
      await otherWallet.signTransaction(SCRIPT_TX_REQUEST);
    const populatedTransaction =
  ...
  it('simulates a transaction', async () => {
    ...
    expect(transactionRequestify.mock.calls[0][0]).toEqual(
      transactionRequestLike,
    );
    ...
    expect(populateTransactionWitnessesSignatureSpy.mock.calls[0][0]).toEqual(
      transactionReq,
    );
  ...
  it('encrypts wallet to keystore', async () => {
    ...
    const encryptKeystoreWalletSpy = vi.spyOn(
      keystoreWMod,
