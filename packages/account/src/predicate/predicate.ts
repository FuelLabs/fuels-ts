...
export class Predicate<TInputData extends InputValue[]> extends Account {
  ...
  constructor({
    ...
    const { predicateBytes, predicateInterface } =
      Predicate.processPredicateData(bytecode, abi, configurableConstants);
  ...
  private getPredicateData(policiesLength: number): Uint8Array {
    ...
    const vmTxMemory = calculateVmTxMemory({
      maxInputs: this.provider
        .getChain()
    ...
    return (
      mainFn?.encodeArguments(this.predicateData, offset) || new Uint8Array()
    );
  }
  ...
  async getResourcesToSpend(
    ...
    return resources.map((resource) => ({
      ...
      padPredicateData: (policiesLength: number) =>
        hexlify(this.getPredicateData(policiesLength)),
    }));
  }
  ...
  private static setConfigurableConstants(
    ...
    try {
      ...
      Object.entries(configurableConstants).forEach(([key, value]) => {
        ...
        const encoded = abiInterface.encodeConfigurable(
          key,
          value as InputValue,
        );
