...
describe("Wallet Manager", () => {
	...
	it("Export privateKey from address from a mnemonic vault", async () => {
		...
		const privateKeyReturned = walletManager.exportPrivateKey(
			accounts[0].address,
		);
	...
	it("Test asserts on method calls", async () => {
		...
		expect(() => walletManager.exportPrivateKey(Address.fromRandom())).toThrow(
			lockedErrMsg,
		);
		...
		const addressErrMsg =
			"No private key found for address the specified wallet address.";

		expect(() => walletManager.getWallet(address)).toThrow(addressErrMsg);
		expect(() => walletManager.exportPrivateKey(address)).toThrow(
			addressErrMsg,
		);
	...
	it("Test if vault secret can be leaked", async () => {
		...
		expect(JSON.stringify(walletManager)).not.toContain(
			WalletManagerSpec.mnemonic,
		);
		expect(JSON.stringify(walletManager.getVaults())).not.toContain(
			WalletManagerSpec.mnemonic,
		);
	});
	...
	it("Wrong password should keep wallet state locked", async () => {
		...
		await walletManager.lock();
		await expect(walletManager.unlock("wrongpass")).rejects.toThrowError(
			"Invalid credentials",
		);
		expect(walletManager.isLocked).toBeTruthy();
	});
