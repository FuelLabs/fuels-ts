...
function parsePath(path: string, depth = 0) {
	...
	return components.map((p) =>
		~p.indexOf(`'`) ? Number.parseInt(p, 10) + HARDENED_INDEX : Number.parseInt(p, 10),
	);
}
...
class HDWallet {
	...
	derivePath(path: string) {
		...
		return paths.reduce(
			(hdwallet, index) => hdwallet.deriveIndex(index),
			<HDWallet>this,
		);
	}
	 ...
	 */
	toExtendedKey(isPublic = false, testnet = false): string {
		if (this.depth >= 256) {
			throw new FuelError(
				ErrorCode.HD_WALLET_ERROR,
		...
		const prefix = getExtendedKeyPrefix(
			this.privateKey == null || isPublic,
			testnet,
		);
	...
	static fromExtendedKey(extendedKey: string) {
		...
		if (bytes.length !== 82 || !isValidExtendedKey(bytes)) {
			throw new FuelError(
				ErrorCode.HD_WALLET_ERROR,
				"Provided key is not a valid extended key.",
			);
		}
		if (!validChecksum) {
			throw new FuelError(
				ErrorCode.HD_WALLET_ERROR,
				"Provided key has an invalid checksum.",
			);
		}
		...
		if (
			(depth === 0 && parentFingerprint !== "0x00000000") ||
			(depth === 0 && index !== 0)
		) {
			throw new FuelError(
				ErrorCode.HD_WALLET_ERROR,
