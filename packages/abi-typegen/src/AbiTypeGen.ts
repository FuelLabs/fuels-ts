...
export class AbiTypeGen {
  ...
  constructor(params: {
    ...
    const { abiFiles, binFiles, outputDir, programType, storageSlotsFiles } =
      params;
    ...
    this.abis = this.abiFiles.map((abiFile) => {
      ...
      const relatedBinFile = this.binFiles.find(
        ({ path }) => path === binFilepath,
      );

      const storageSlotFilepath = abiFile.path.replace(
        '-abi.json',
        '-storage_slots.json',
      );
