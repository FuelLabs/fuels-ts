...
describe('EvmAddressType.ts', () => {
  test('should properly parse type attributes', () => {
    ...
    const suitableForEvmAddress = EvmAddressType.isSuitableFor({
      type: EvmAddressType.swayType,
    });
    const suitableForStruct = EvmAddressType.isSuitableFor({
      type: StructType.swayType,
    });
    const suitableForVector = EvmAddressType.isSuitableFor({
    ...
    expect(evmAddress.requiredFuelsMembersImports).toStrictEqual([
      'EvmAddress',
    ]);
  });
});
