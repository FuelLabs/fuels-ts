...
describe('TupleType.ts', () => {
  test('should properly parse type attributes', () => {
    ...
    const suitableForTuple = TupleType.isSuitableFor({
      type: TupleType.swayType,
    });
    const suitableForArray = TupleType.isSuitableFor({
      type: ArrayType.swayType,
    });
    ...
    expect(c.attributes.outputLabel).toEqual(
      '[number, StructAOutput<StructBOutput<BN>, string>]',
    );
