...
describe('StructType.ts', () => {
  test('should properly parse type attributes', () => {
    const parseTypeArguments = vi.spyOn(
      parseTypeArgumentsMod,
      'parseTypeArguments',
    );
    ...
    const suitableForStruct = StructType.isSuitableFor({
      type: StructType.swayType,
    });
    const suitableForU16 = StructType.isSuitableFor({ type: U16Type.swayType });
    const suitableForEvmAddress = StructType.isSuitableFor({
      type: EvmAddressType.swayType,
    });
    const suitableForBytes = StructType.isSuitableFor({
      type: BytesType.swayType,
    });
    const suitableForStdString = StructType.isSuitableFor({
    ...
    expect(inputs).toEqual(
      'propC1: StructAInput<StructBInput<BigNumberish>, BigNumberish>',
    );

    let outputs = c.getStructContents({ types, target: TargetEnum.OUTPUT });
    expect(outputs).toEqual(
      'propC1: StructAOutput<StructBOutput<number>, number>',
    );
