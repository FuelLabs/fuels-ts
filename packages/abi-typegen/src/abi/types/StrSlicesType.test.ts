...
describe("StrSlicesType.ts", () => {
	test("should properly parse type attributes", () => {
		...
		const suitableForStrSlices = StrSliceType.isSuitableFor({
			type: StrSliceType.swayType,
		});
		const suitableForU16 = StrSliceType.isSuitableFor({
			type: BoolType.swayType,
		});
		const suitableForStr = StrSliceType.isSuitableFor({
			type: StrType.swayType,
		});
		const suitableForStdString = StrSliceType.isSuitableFor({
