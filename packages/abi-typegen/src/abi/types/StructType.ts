...
export class StructType extends AType implements IType {
  ...
  static IGNORE_REGEX: RegExp =
    /^struct (Vec|RawVec|EvmAddress|Bytes|String)$/m;
  ...
  public getStructDeclaration(params: { types: IType[] }) {
    ...
    if (typeParameters) {
      const structs = typeParameters.map((typeId) =>
        findType({ types, typeId }),
      );

      const labels = structs.map(
        ({ attributes: { inputLabel } }) => inputLabel,
      );
