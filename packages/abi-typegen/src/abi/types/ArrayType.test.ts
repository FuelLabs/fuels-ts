...
describe("ArrayType.ts", () => {
	...
	test("should properly evaluate type suitability", () => {
		const suitableForTuple = ArrayType.isSuitableFor({
			type: TupleType.swayType,
		});
		const suitableForArray = ArrayType.isSuitableFor({
	...
	test("should properly parse type attributes: simple", () => {
		...
		const project = getTypegenForcProject(
			AbiTypegenProjectsEnum.STRUCT_WITH_ARRAY,
		);
	...
	test("should properly parse type attributes: nested", () => {
		...
		const project = getTypegenForcProject(
			AbiTypegenProjectsEnum.ARRAY_WITH_GENERICS,
		);
