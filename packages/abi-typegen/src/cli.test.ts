...
describe('cli.ts', () => {
  ...
  test('should error if called with incompatible parameters: -s, -c', async () => {
    ...
    const err1 =
      /error: option '-c, --contract' cannot be used with option '-s, --script/;
    expect(err.mock.calls[0][0].toString()).toMatch(err1);

    const err2 =
      /error: option '-s, --script' cannot be used with option '-c, --contract/m;
  ...
  test('should error if called with incompatible parameters: -s, -p', async () => {
    ...
    const err1 =
      /error: option '-s, --script' cannot be used with option '-p, --predicate/m;
    expect(err.mock.calls[0][0].toString()).toMatch(err1);

    const err2 =
      /error: option '-p, --predicate' cannot be used with option '-s, --script/;
    expect(err.mock.calls[1][0].toString()).toMatch(err2);
  });
  ...
  test('should error if called with incompatible parameters: -p, -c', async () => {
    ...
    const err1 =
      /error: option '-c, --contract' cannot be used with option '-p, --predicate/m;
    expect(err.mock.calls[0][0].toString()).toMatch(err1);

    const err2 =
      /error: option '-p, --predicate' cannot be used with option '-c, --contract/;
    expect(err.mock.calls[1][0].toString()).toMatch(err2);
  });
