import type { EnumType, RawTypeHash } from '../../abi/types/EnumType';
...
export function formatEnums(params: { types: IType[] }) {
  ...
  const typeHash: RawTypeHash = types.reduce(
    (hash, row) => ({
      ...hash,
  ...
  const enums = types
    ...
    .map((t) => {
      ...
      const inputNativeValues = et.getNativeEnum(typeHash);
      const outputNativeValues = et.getNativeEnum(typeHash);
