...
describe('Wallet', () => {
  ...
  it.only('get min_gas for transaction', async () => {
    const sender = await generateTestWallet(provider, [[500_000, BaseAssetId]]);
    ...
    console.log(minGas.toString());
    // request.gasLimit = maxGasPerTx.sub(minGas);
    // 456
    // 7084
    const result = await provider.getTransactionCost(request);
    console.log(result);

    // const transactionSigned = await sender.populateTransactionWitnessesSignature(request);
    // const result = await provider.sendTransaction(transactionSigned);
    // const final = await result.waitForResult();
