...
describe('Wallet', () => {
  ...
  it('can withdraw an amount of base asset', async () => {
    ...
    const sender = await generateTestWallet(provider, [[100, NativeAssetId]]);
    const recipientHexAddress =
      '0x00000000000000000000000047ba61eec8e5e65247d717ff236f504cf3b0a263';
    const recipient = Bech32.fromB256(recipientHexAddress);
    const amount = 10;
    ...
    expect(recipientHexAddress).toEqual(messageOutReceipt.recipient);
  ...
  it('can withdraw an amount of base asset using mutiple uxtos', async () => {
    ...
    await seedTestWallet(sender, [[100, NativeAssetId]]);
    await seedTestWallet(sender, [[100, NativeAssetId]]);
    const recipientHex = '0x00000000000000000000000047ba61eec8e5e65247d717ff236f504cf3b0a263';
    const recipient = Bech32.fromB256(recipientHex);
    const amount = 110;
    const tx = await sender.withdrawToBaseLayer(recipient, amount);
    const result = await tx.wait();
    ...
    const messageOutReceipt = <TransactionResultMessageOutReceipt>result.receipts[0];
    expect(result.transactionId).toEqual(messageOutReceipt.sender);
    expect(recipientHex).toEqual(messageOutReceipt.recipient);
    expect(amount.toString()).toEqual(messageOutReceipt.amount.toString());
