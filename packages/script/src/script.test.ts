...
import { ScriptProjectsEnum, getScriptForcProject } from '../test/fixtures';
import { jsonAbiFragmentMock, jsonAbiMock } from '../test/mocks';

import { Script } from './index';

const { abiContents: scriptJsonAbi, binHexlified: scriptBin } =
  getScriptForcProject(ScriptProjectsEnum.CALL_TEST_SCRIPT);
...
describe('Script', () => {
  ...
  it('can call a script', async () => {
    ...
    const { transactionResult, result } = await callScript(
      wallet,
      scriptRequest,
      input,
    );
