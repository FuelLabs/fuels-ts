...
import { FunctionInvocationScope, ScriptRequest } from '@fuel-ts/program';
...
> extends FunctionInvocationScope<TArgs, TReturn> {
  ...
  private buildScriptRequest() {
    ...
    const maxInputs =
      chainInfoCache.consensusParameters.txParameters.maxInputs.toNumber();
    ...
    this.scriptRequest = new ScriptRequest(
      ...
      (args: TArgs) =>
        this.func.encodeArguments(
          args,
          ScriptRequest.getScriptDataOffsetWithScriptBytes(
            byteLength,
            maxInputs,
          ),
        ),
