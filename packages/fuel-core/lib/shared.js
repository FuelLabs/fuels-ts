    ...
    throw new Error(
      `Unsupported platform ${process.platform}.${
        process.platform === 'win32'
          ? ' If you are on Windows, please use WSL.'
          : ''
      }`,
    )
  }
...
export const isGitBranch = (versionFileContents) =>
...
export const buildFromGitBranch = (branchName) => {
  ...
  if (existsSync(fuelCoreRepoDir)) {
    execSync(
      `cd ${fuelCoreRepoDir} && git pull && git checkout ${branchName}`,
      stdioOpts,
    )
    execSync(`cd ${fuelCoreRepoDir} && cargo build`, stdioOpts)
  } else {
    execSync(
      `git clone --branch ${branchName} ${fuelCoreRepoUrl} ${fuelCoreRepoDir}`,
      stdioOpts,
    )
    execSync(`cd ${fuelCoreRepoDir} && cargo build`, stdioOpts)
  }
