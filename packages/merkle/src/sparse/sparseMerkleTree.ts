...
import { hashLeaf, hashNode, isLeaf, parseLeaf, parseNode } from './treeHasher';
...
export class SparseMerkleTree {
  ...
  updateWithSideNodes(
    ...
    for (let i = 0; i < MAX_HEIGHT; i += 1) {
      ...
      if (
        i - offsetOfSideNodes < 0 ||
        sideNodes[i - offsetOfSideNodes] === ''
      ) {
        if (
          commonPrefixCount !== MAX_HEIGHT &&
          commonPrefixCount > MAX_HEIGHT - 1 - i
        ) {
          sideNode = ZERO;
        } else {
          continue;
  ...
  update(key: string, value: string): void {
    const [sideNodes, oldLeafHash, oldLeafData] = this.sideNodesForRoot(
      key,
      this.root,
    );
    const newRoot = this.updateWithSideNodes(
      key,
      value,
      sideNodes,
      oldLeafHash,
  ...
  delete(key: string): void {
    ...
    const newRoot = this.deleteWithSideNodes(
      key,
      sideNodes,
      oldLeafHash,
      oldLeafData,
  ...
  prove(key: string): SparseMerkleProof {
    ...
    const proof = new SparseMerkleProof(
      nonEmptySideNodes,
