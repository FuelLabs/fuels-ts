...
describe("UpgradePurposeCoder", () => {
	const bytecodeRoot =
		"0x4dd439ec50a1caa950456cefb548043d144681e99361ab9ca6939fc5e466bde0";
	const checksum =
		"0xc343ed6a0396c31c21bcbd77f24fb0300ab6fcbb177da5cbd0b34c8695e72d2d";
	...
	it("can encode UpgradePurpose [ConsensusParameters]", () => {
		...
		const expectedEncoded = Uint8Array.from([
			...typeBytes,
			...witnessIndexBytes,
			...checksumBytes,
		]);
	...
	it("can encode UpgradePurpose [StateTransition]", () => {
		...
		const typeBytes = [
			...
			0,
			0,
			UpgradePurposeTypeEnum.StateTransition,
		];
	...
	it("can decode UpgradePurpose [ConsensusParameters]", () => {
		...
		const [upgradePurpose] = new UpgradePurposeCoder().decode(
			arrayify(encodedHex),
			0,
		);
