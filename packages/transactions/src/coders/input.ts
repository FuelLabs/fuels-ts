...
export class InputCoinCoder extends Coder<InputCoin, InputCoin> {
	...
	encode(value: InputCoin): Uint8Array {
		...
		parts.push(
			new ByteArrayCoder(value.predicateLength.toNumber()).encode(
				value.predicate,
			),
		);
		parts.push(
			new ByteArrayCoder(value.predicateDataLength.toNumber()).encode(
				value.predicateData,
			),
		);
	...
	decode(data: Uint8Array, offset: number): [InputCoin, number] {
		...
		[decoded, o] = new ByteArrayCoder(predicateLength.toNumber()).decode(
			data,
			o,
		);
		const predicate = decoded;
		[decoded, o] = new ByteArrayCoder(predicateDataLength.toNumber()).decode(
			data,
			o,
		);
...
export class InputMessageCoder extends Coder<InputMessage, InputMessage> {
	...
	static getMessageId(
		value: Pick<
			InputMessage,
			"sender" | "recipient" | "nonce" | "amount" | "data"
		>,
	): string {
		const parts: Uint8Array[] = [];
