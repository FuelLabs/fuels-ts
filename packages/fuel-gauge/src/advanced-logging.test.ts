...
describe("Advanced Logging", () => {
	...
	it("can get log data from require [condition=false]", async () => {
		const invocation = advancedLogContract.functions.test_function_with_require(
			1,
			3,
		);
		try {
			...
			if ((<Error>error).message) {
				...
				throw new Error(
					'it should be possible to decode error from "require" statement',
				);
			}
		}
	});
	...
	describe("should properly decode all logs in a multicall with inter-contract calls", () => {
		const setupCallTest = getSetupContract(
			FuelGaugeProjectsEnum.CALL_TEST_CONTRACT,
		);
		const setupConfigurable = getSetupContract(
			FuelGaugeProjectsEnum.CONFIGURABLE_CONTRACT,
		);
		const setupCoverage = getSetupContract(
