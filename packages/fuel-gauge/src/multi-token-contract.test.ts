...
describe("MultiTokenContract", () => {
	it(
		...
		async () => {
			...
			const { transactionResult } = await multiTokenContract
				.multiCall(
					subIds.map((subId) =>
						multiTokenContract.functions.mint_coins(
							subId,
			...
			const validateMintPromises = subIds.map(async (subId) => {
				expect(
					bn(
						await getBalance(contractId, helperDict[subId].assetId),
					).toNumber(),
				).toBe(helperDict[subId].amount);
			});
			...
			const validateTransferPromises = subIds.map(async (subId) => {
				...
				expect(
					bn(await userWallet.getBalance(helperDict[subId].assetId)).toNumber(),
				).toBe(helperDict[subId].amount);
	...
	it("can burn coins", async () => {
		...
		const validateMintPromises = subIds.map(async (subId) => {
			expect(
				bn(await getBalance(contractId, helperDict[subId].assetId)).toNumber(),
			).toBe(helperDict[subId].amount);
		});
		...
		await multiTokenContract
			.multiCall(
				subIds.map((subId) =>
					multiTokenContract.functions.burn_coins(
						subId,
						helperDict[subId].amountToBurn,
					),
				),
			)
			.call();
		...
		const validateBurnPromises = subIds.map(async (subId) => {
			...
			expect(
				bn(await getBalance(contractId, helperDict[subId].assetId)).toNumber(),
			).toBe(helperDict[subId].amount - helperDict[subId].amountToBurn);
		});
