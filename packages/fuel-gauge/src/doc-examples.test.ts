...
const ADDRESS_BYTES = new Uint8Array([
  241, 233, 44, 66, 185, 9, 52, 170, 99, 114, 227, 11, 197, 104, 163, 38, 246,
  230, 106, 26, 2, 136, 89, 94, 110, 63, 189, 57, 42, 79, 62, 110,
]);
...
describe('Doc Examples', () => {
  ...
  test('it has Address tools', () => {
    ...
    expect(addressCloneFromBech.toString()).toEqual(
      addressCloneFromB256.toString(),
    );
  ...
  test('it has conversion tools', async () => {
    ...
    expect(arrayify(assetId)).toEqual(
      arrayify(Address.fromB256(assetId).toB256()),
    );
  });
  ...
  test('it can work with wallets', async () => {
    ...
    let unlockedWallet: WalletUnlocked = lockedWallet.unlock(privateKey);
    // or directly from a private key
    unlockedWallet = Wallet.fromPrivateKey(privateKey, provider);
  ...
  it('can create a predicate and use', async () => {
    ...
    const wallet1: WalletUnlocked = Wallet.fromPrivateKey(
      privateKey1,
      provider,
    );
    ...
    const wallet3: WalletUnlocked = Wallet.fromPrivateKey(
      privateKey3,
      provider,
    );
    ...
    await seedTestWallet(wallet1, [
      { assetId: baseAssetId, amount: bn(1_000_000) },
    ]);
    await seedTestWallet(wallet2, [
      { assetId: baseAssetId, amount: bn(2_000_000) },
    ]);
    await seedTestWallet(wallet3, [
      { assetId: baseAssetId, amount: bn(300_000) },
    ]);
