...
import type { Contract, TransactionResultReceipt, WalletUnlocked } from 'fuels';
import {
  ContractFactory,
  FUEL_NETWORK_URL,
  Provider,
...
describe('Revert Error Testing', () => {
  ...
  beforeAll(async () => {
    ...
    const { binHexlified: bytecode, abiContents: FactoryAbi } =
      getFuelGaugeForcProject(FuelGaugeProjectsEnum.REVERT_ERROR);
  ...
  it('can pass require checks [valid]', async () => {
    const inputPrice = bn(10);
    const inputTokenId = bn(100);

    const { logs } = await contractInstance.functions
      .validate_inputs(inputTokenId, inputPrice)
      .call();
    ...
    expect(
      ...
      {
        token_id: inputTokenId.toString(),
  ...
  it('should throw for a missing OutputChange', async () => {
    const { binHexlified: tokenBytecode, abiContents: tokenAbi } =
