...
describe('CallTestContract', () => {
  it.each([0, 1337, U64_MAX.sub(1)])(
    'can call a contract with u64 (%p)',
    async (num) => {
  ...
  it('can call a function with empty arguments', async () => {
    ...
    const { value: emptyThenValue } = await contract.functions
      .empty_then_value(35)
      .call();
    expect(emptyThenValue.toHex()).toEqual(toHex(63));

    const { value: valueThenEmpty } = await contract.functions
      .value_then_empty(35)
      .call();
  ...
  it('can make multiple calls', async () => {
    ...
    const multiCallScope = contract.multiCall([
      invocationA,
