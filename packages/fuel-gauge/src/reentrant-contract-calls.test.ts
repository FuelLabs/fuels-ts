...
const storageTest = getFuelGaugeForcProject(
...
describe('Reentrant Contract Calls', () => {
  ...
  beforeAll(async () => {
    ...
    const factoryBar = new ContractFactory(
      bar.binHexlified,
      bar.abiContents,
      wallet,
    );
    ...
    const factoryFoo = new ContractFactory(
      foo.binHexlified,
      foo.abiContents,
      wallet,
    );
  ...
  it('should ensure the SDK returns the proper value for a reentrant call', async () => {
    ...
    const returnReceipts = receipts.filter(
      (r) =>
        r.type === ReceiptType.ReturnData && r.id === fooContract.id.toB256(),
    );
  ...
  it('should ensure the SDK returns the proper value for a reentrant call on multi-call', async () => {
    ...
    const result = await fooContract
      .multiCall([
        reentrantCall,
        storageContract.functions.return_var3(),
        reentrantCall,
      ])
      .call();
