...
describe('Policies', () => {
  ...
  const validatePolicies = ({
    ...
    expect(
      bn(transaction.policies?.[1].data).eq(bn(params.witnessLimit)),
    ).toBeTruthy();
    ...
    expect(
      bn(transaction.policies?.[3]?.data).lte(bn(params.maxFee)),
    ).toBeTruthy();
  };
  ...
  it('should ensure TX policies are properly set (CreateTransactionRequest)', async () => {
    ...
    const { transactionRequest: txRequest } =
      factory.createTransactionRequest(txParams);
  ...
  describe('should ensure TX maxFee policy limits TX execution as expected', () => {
    it('on account transfer', async () => {
      ...
      await expect(async () => {
        ...
        new RegExp(`Max fee '${maxFee}' is lower than the required`),
      );
    });
    ...
    it('on account withdraw to base layer', async () => {
      ...
      await expect(async () => {
        const pendingTx = await wallet.withdrawToBaseLayer(
          receiver.address,
          500,
          txParams,
        );
