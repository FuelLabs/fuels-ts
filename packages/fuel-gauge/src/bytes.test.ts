...
describe('Bytes Tests', () => {
  ...
  it('should test bytes output', async () => {
    ...
    const { value } = await contractInstance.functions.return_bytes(INPUT).callAndWait<number[]>();
  ...
  it('should test bytes input', async () => {
    ...
    const { value } = await contractInstance.functions.accept_bytes(INPUT).callAndWait<number[]>();
  ...
  it('should test bytes input [script-bytes]', async () => {
    ...
    const { value } = await scriptInstance.functions.main(1, INPUT).callAndWait<BN>();
