...
import { FUEL_NETWORK_URL, Provider, Script, bn } from 'fuels';
...
describe('Script Coverage', () => {
  const { binHexlified: scriptBin, abiContents: scriptAbi } =
  ...
  it('can call script and use main arguments [two args, read logs]', async () => {
    const wallet = await setup();
    const scriptInstance = getScript<[BigNumberish, Baz], Baz>(
