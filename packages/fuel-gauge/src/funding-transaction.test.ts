...
describe(__filename, () => {
  ...
  const fundingTxWithMultipleUTXOs = async ({
    ...
    const resources = await mainWallet.getResourcesToSpend([
      [totalAmount + 2_000, baseAssetId],
    ]);
  ...
  it('should not fund a transaction request when it is already funded', async () => {
    ...
    expect(
      bn((<CoinTransactionRequestInput>request.inputs[0]).amount).toNumber(),
    ).toBe(1000);
  ...
  it('should ensure a partially funded Transaction will require only missing funds', async () => {
    ...
    expect(baseAssetResource?.amount.toString()).toBe(
      totalInBaseAsset.toString(),
    );
    expect(assetAResource?.amount.toString()).toBe(
      partiallyInAssetA.toString(),
    );
  ...
  it('should ensure a funded Transaction will not require more funds from another user', async () => {
    ...
    transactionRequest =
      (await fundedWallet.populateTransactionWitnessesSignature(
        transactionRequest,
      )) as ScriptTransactionRequest;
