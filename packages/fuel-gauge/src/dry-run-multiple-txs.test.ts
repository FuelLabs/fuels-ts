...
describe("dry-run-multiple-txs", () => {
	...
	const { abiContents: abiRevert, binHexlified: binRevert } =
		getFuelGaugeForcProject(FuelGaugeProjectsEnum.REVERT_ERROR);
	const { abiContents: abiMultiToken, binHexlified: binMultiToken } =
		getFuelGaugeForcProject(FuelGaugeProjectsEnum.MULTI_TOKEN_CONTRACT);
	const { abiContents: abiLog, binHexlified: binLog } = getFuelGaugeForcProject(
		FuelGaugeProjectsEnum.ADVANCED_LOGGING,
	);
	const { abiContents: abiLogOther, binHexlified: binLogOther } =
		getFuelGaugeForcProject(
	...
	const deployContracts = async () => {
		...
		const multiTokenFactory = new ContractFactory(
			binMultiToken,
			abiMultiToken,
			wallet,
		);
		...
		const logOtherFactory = new ContractFactory(
			binLogOther,
			abiLogOther,
			wallet,
		);
