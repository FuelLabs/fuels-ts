...
describe("TransactionSummary", () => {
	...
	const verifyTransactionSummary = (params: {
		...
		expect(transaction.receipts).toStrictEqual(
		...
		if (!isRequest) {
			expect((<TransactionResult>transaction).gqlTransaction).toStrictEqual(
				expect.any(Object),
			);
	...
	it("should ensure getTransactionSummary executes just fine", async () => {
		...
		expect(transactionSummary.transaction).toStrictEqual(
	...
	it("should ensure getTransactionsSummaries executes just fine", async () => {
		...
		const tx1 = await adminWallet.transfer(
			sender.address,
			500_000,
			baseAssetId,
			{
	...
	it("should ensure getTransactionSummaryFromRequest executes just fine", async () => {
		...
		const transactionRequest =
			await adminWallet.populateTransactionWitnessesSignature(request);
	...
	describe("Transfer Operations", () => {
		const setupContract = getSetupContract(
		...
		const validateTransferOperation = (params: {
			...
			recipients: {
				...
				quantities: { amount: number; assetId: string }[];
			}[];
			...
			recipients.forEach(({ address, quantities }, index) => {
				...
				quantities.forEach(({ amount, assetId }, qunatitiesIndex) => {
					expect(
						Number(operations[index].assetsSent?.[qunatitiesIndex].amount),
					).toBe(amount);
					expect(operations[index].assetsSent?.[qunatitiesIndex].assetId).toBe(
						assetId,
					);
				});
			});
		...
		it("should ensure transfer operation is assembled (ACCOUNT TRANSFER TO CONTRACT)", async () => {
			...
			const tx1 = await wallet.transferToContract(
				contract1.id,
				amount,
				ASSET_A,
			);
			...
			validateTransferOperation({
				...
				recipients: [
					{ address: contract1.id, quantities: [{ amount, assetId: ASSET_A }] },
				],
			});
		});
		...
		it("should ensure transfer operation is assembled (CONTRACT TRANSFER TO ACCOUNT)", async () => {
			...
			validateTransferOperation({
				...
				toType: AddressType.account,
				recipients: [
					{ address: recipient.address, quantities: [{ amount, assetId }] },
				],
			});
		});
		...
		it("should ensure transfer operation is assembled (CONTRACT TRANSFER TO CONTRACT)", async () => {
			...
			validateTransferOperation({
				...
				toType: AddressType.contract,
				recipients: [
					{ address: contractRecipient.id, quantities: [{ amount, assetId }] },
				],
			});
		});
