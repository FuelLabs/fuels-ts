...
async function verifyKeepAliveMessageWasSent(
  subscriptionStream: ReadableStream<Uint8Array>,
) {
...
function getSubscriptionStreamFromFetch(streamHolder: {
...
describe('TransactionResponse', () => {
  ...
  it('should ensure getTransactionSummary fetchs a transaction and assembles transaction summary', async () => {
    ...
    const nodeProvider = await Provider.create(
      `http://${ip}:${port}/v1/graphql`,
    );
  ...
  it('should ensure waitForResult always waits for the transaction to be processed', async () => {
    ...
    const response = await TransactionResponse.create(
      transactionId,
      nodeProvider,
    );
