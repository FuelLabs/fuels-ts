...
const AltToken = '0xab78cd0110e376fda5903820ff39375a9e9ea3374bba8a8f5b153a4bbc0a63bc';
...
describe(
  ...
  () => {
    ...
    it('Single call with forwarding a alt token', async () => {
      ...
      const amountToForward = 0.01;

      const { waitForResult } = await contract.functions
        .return_context_amount()
        .callParams({
          forward: [amountToForward, AltToken],
          gasLimit: 1000000,
        })
        .txParams({
          gasLimit: 3000000,
      ...
      const { value } = await waitForResult();
      expect(value.toHex()).toEqual(toHex(amountToForward));
    });
    ...
    it('should transfer asset to a deployed contract just fine (NATIVE ASSET)', async () => {
      ...
      const initialBalance = new BN(
        await contract.getBalance(provider.getBaseAssetId())
      ).toNumber();
