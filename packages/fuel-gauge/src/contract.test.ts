...
describe('Contract', () => {
  it('generates function methods on a complex contract', async () => {
    ...
    const contract = new Contract(ZeroBytes32, CallTestContractAbi__factory.abi, wallet);

    const fragment = contract.interface.getFunction('sum');
    const interfaceSpy = vi.spyOn(fragment, 'encodeArguments');

    try {
      await contract.functions.sum('1qwew', 2);
    } catch {
  ...
  it('assigns a provider if passed', async () => {
    ...
    const contract = new Contract(getRandomB256(), CallTestContractAbi__factory.abi, provider);
