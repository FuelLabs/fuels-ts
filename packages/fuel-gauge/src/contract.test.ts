...
const { binHexlified: contractBytecode, abiContents: abi } =
  getFuelGaugeForcProject(FuelGaugeProjectsEnum.CALL_TEST_CONTRACT);
...
describe('Contract', () => {
  ...
  it('adds multiple contracts on invocation', async () => {
    ...
    const scope = contract.functions.call_external_foo(
      1336,
      otherContract.id.toB256(),
    );
  ...
  it('adds multiple contracts on multicalls', async () => {
    ...
    expect(JSON.stringify(results)).toEqual(
      JSON.stringify([bn(1337), bn(1338)]),
    );
  });
  ...
  it('submits multiple calls', async () => {
    ...
    const { value: results } = await contract
      ...
      .call();
    expect(JSON.stringify(results)).toEqual(
      JSON.stringify([bn(1337), bn(1337)]),
    );
  });
  ...
  it('simulates multiple calls', async () => {
    ...
    expect(callResult.receipts).toEqual(
  ...
  it('MultiCall with multiple forwarding', async () => {
    ...
    expect(JSON.stringify(value)).toEqual(
      JSON.stringify([bn(100), bn(200), AltToken]),
    );
  });
  ...
  it('can forward gas to multicall calls', async () => {
    ...
    expect(value[1].toNumber()).toBeGreaterThanOrEqual(
      1_000_000 * minThreshold,
    );
  ...
  it('Fail before submit if gasLimit is lower than gasUsed', async () => {
    ...
    const invocationScope = contract.functions
      .return_context_amount()
      .callParams({
        forward: [100, baseAssetId],
      });
    ...
    await expect(
      ...
      new RegExp(`Gas limit '${gasLimit}' is lower than the required: `),
    );
  });
  ...
  it('Parse TX to JSON and parse back to TX', async () => {
    ...
    const invocationScopes = [
      contract.functions.foo(num),
      contract.functions.boo(struct),
    ];
    ...
    const response = await contract.account?.sendTransaction(
      transactionRequestParsed,
    );
    const {
      value: [resultA, resultB],
    } = await FunctionInvocationResult.build<any>(
      invocationScopes,
      response,
      true,
      contract,
  ...
  it('Provide a custom provider and public wallet to the contract instance', async () => {
    ...
    class ProviderCustom extends Provider {
      ...
      async sendTransaction(
        ...
        signedTransaction =
          await externalWallet.signTransaction(transactionRequest);
    ...
    contract.account = Wallet.fromAddress(
      externalWallet.address,
      customProvider,
    );
    ...
    const txCost = await contract.provider.getTransactionCost(
      transactionRequestParsed,
    );
    ...
    expect(transactionResult.transaction.witnesses[0].data).toEqual(
      signedTransaction,
    );
    expect(resultA.toHex()).toEqual(bn(num).add(1).toHex());
    expect(resultB.a).toEqual(!struct.a);
  ...
  it('should ensure multicall allows multiple heap types', async () => {
    ...
    expect(JSON.stringify(value)).toBe(
      JSON.stringify([bn(0), vector, new Uint8Array()]),
    );
  });
  ...
  it('should tranfer asset to a deployed contract just fine (NATIVE ASSET)', async () => {
    const wallet = await generateTestWallet(provider, [
      [10_000_000, baseAssetId],
    ]);

    const contract = await setupContract();

    const initialBalance = new BN(
      await contract.getBalance(baseAssetId),
    ).toNumber();
  ...
  it('should ensure "get" can be used to execute a contract call without a wallet', async () => {
    ...
    const contractToCall = new Contract(
      contract.id,
      contract.interface,
      contract.provider,
    );
  ...
  it('should ensure "get" does not modify the blockchain state', async () => {
    ...
    await storageContract.functions
      .initialize_counter(initialCounterValue)
      .get();
  ...
  it('should ensure "maxFee" and "gasLimit" can be set for a contract call', async () => {
    ...
    const maxFeePolicy = transaction.policies?.find(
      (policy) => policy.type === PolicyType.MaxFee,
    );
