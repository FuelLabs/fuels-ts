...
import { assertBalance, fundPredicate } from './utils/predicate';
...
describe('Predicate', () => {
  ...
  const {
    binHexlified: predicateBytesConfigurable,
    abiContents: predicateAbiConfigurable,
  } = getFuelGaugeForcProject(
    FuelGaugeProjectsEnum.PREDICATE_WITH_CONFIGURABLE,
  );
  ...
  describe('Configurables', () => {
    ...
    const defaultValues = {
      FEE: 10,
      ADDRESS:
        '0x38966262edb5997574be45f94c665aedb41a1663f5b0528e765f355086eebf96',
    };
    ...
    it('throws when configurable data is not set', async () => {
      ...
      await expect(
        predicate.transfer(destination.address, 300, baseAssetId, {
          gasLimit: 1000,
        }),
