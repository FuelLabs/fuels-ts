import type { BigNumberish, WalletLocked, WalletUnlocked } from 'fuels';
import { FUEL_NETWORK_URL, Predicate, Provider, toHex } from 'fuels';
...
import { assertBalances, fundPredicate, setupWallets } from './utils/predicate';
...
describe('Predicate', () => {
  const {
    binHexlified: predicateBytesAddress,
    abiContents: predicateAbiMainArgsAddress,
  ...
  const {
    binHexlified: predicateBytesMainArgsVector,
    abiContents: predicateAbiMainArgsVector,
  ...
  const {
    binHexlified: predicateBytesMainArgsU32,
    abiContents: predicateAbiMainArgsU32,
  ...
  describe('Arguments', () => {
    ...
    it('calls a predicate with invalid address data and returns false', async () => {
      ...
      await expect(
        predicate.transfer(receiver.address, 50, baseAssetId, {
          gasLimit: 1000,
        }),
