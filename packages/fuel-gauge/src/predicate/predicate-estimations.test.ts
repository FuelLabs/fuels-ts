...
describe("Predicate", () => {
	...
	const {
		binHexlified: predicateBytesMainArgsStruct,
		abiContents: predicateAbiMainArgsStruct,
	...
	describe("Estimate predicate gas", () => {
		...
		it("estimatePredicates should assign gas to the correct input", async () => {
			...
			(<CoinTransactionRequestInput>tx.inputs[0]).predicate =
				predicateStruct.bytes;
			...
			const contract: ContractTransactionRequestInput = {
				type: 1,
				contractId:
					"0xb64fa600c9a940529020fd47a18f9c1395946fbf636aad13fe029db4820ed354",
			...
			const nonPredicateUtxo: CoinTransactionRequestInput = {
				...
				owner:
					"0xd8813d1f9ca165ce2e8710382c3d65d64e7bd43c0f7a3d51689bcdf9513411cd",
				type: 0,
				txPointer: "0x00000000000000000000000000000000",
				witnessIndex: 0,
			};
			...
			const predicateCoinInput = <MessageTransactionRequestInput>(
				txEstimated.inputs[0]
			);
			expect(Number(predicateCoinInput.predicateGasUsed)).toBeGreaterThan(1);
			const predicateMessageInput = <MessageTransactionRequestInput>(
				txEstimated.inputs[1]
			);
			expect(Number(predicateMessageInput.predicateGasUsed)).toBeGreaterThan(1);
			const nonPredicateMessageInput = <MessageTransactionRequestInput>(
				txEstimated.inputs[2]
			);
			expect(nonPredicateMessageInput.predicateGasUsed).toBeUndefined();
			const contractInput = <ContractTransactionRequestInput>(
				txEstimated.inputs[3]
			);
			expect(contractInput.contractId).toBe(contract.contractId);
			const nonPredicateInput = <CoinTransactionRequestInput>(
				txEstimated.inputs[4]
			);
		...
		test("transferring funds from a predicate estimates the predicate and does only one dry run", async () => {
			...
			const estimatePredicatesSpy = vi.spyOn(
				provider.operations,
				"estimatePredicates",
			);
