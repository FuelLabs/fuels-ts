...
function getGasUsage(callResult: CallResult) {
  const scriptResult = callResult.receipts.find(
    (r) => r.type === ReceiptType.ScriptResult,
  ) as ReceiptScriptResult | undefined;
...
export class InvocationResult<T = any> {
  ...
  protected getDecodedValue(callResult: CallResult) {
    ...
    if (
      this.functionScopes.length === 1 &&
      callConfig &&
      'bytes' in callConfig.program
    ) {
      return callResultToInvocationResult<T>(callResult, callConfig, logs);
    }

    const outputAsIterableHeap =
      (callConfig?.func.encoding === ENCODING_V0 &&
...
> extends InvocationResult<T> {
  ...
  static async build<T, TTransactionType = void>(
    ...
    const txResult =
      await transactionResponse.waitForResult<TTransactionType>();
...
export class InvocationCallResult<T = any> extends InvocationResult<T> {
  ...
  static async build<T>(
    ...
    const fnResult = await new InvocationCallResult<T>(
      funcScopes,
      callResult,
      isMultiCall,
    );
