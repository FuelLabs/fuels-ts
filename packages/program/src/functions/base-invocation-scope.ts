...
function createContractCall(
  ...
  const { program, args, forward, func, callParameters, externalAbis } =
    funcScope.getCallConfig();
  const dataPointerOffset = funcScope.getCallConfig().func.isInputDataPointer
    ? POINTER_DATA_OFFSET
    : 0;
  const data = func.encodeArguments(
    args as Array<InputValue>,
    offset + dataPointerOffset,
  );
...
export class BaseInvocationScope<TReturn = any> {
  ...
  protected hasCallParamsGasLimit = false; // flag to check if any of the callParams has gasLimit set
  ...
  protected get calls() {
    ...
    const maxInputs =
      consensusParams.consensusParameters.txParameters.maxInputs;
    const script = getContractCallScript(
    ...
    return this.functionInvocationScopes.map((funcScope) =>
      createContractCall(
        funcScope,
        script.getScriptDataOffset(maxInputs.toNumber()),
      ),
    );
  }
  ...
  protected updateScriptRequest() {
    ...
    const contractCallScript = getContractCallScript(
      this.functionInvocationScopes,
      maxInputs,
    );
    ...
    calls.forEach((c) => {
      ...
      if (c.externalContractsAbis) {
        Object.keys(c.externalContractsAbis).forEach((contractId) =>
          this.transactionRequest.addContractInputAndOutput(
            Address.fromB256(contractId),
          ),
        );
      }
    });
  ...
  protected async prepareTransaction() {
    ...
    if (this.transactionRequest.type === TransactionType.Script) {
      this.transactionRequest.abis = getAbisFromAllCalls(
        this.functionInvocationScopes,
      );
    }
  }
  ...
  protected checkGasLimitTotal() {
    const gasLimitOnCalls = this.calls.reduce(
      (total, call) => total.add(call.gas || 0),
      bn(0),
    );
  ...
  txParams(txParams: TxParams) {
    ...
    request.witnessLimit = txParams.witnessLimit
      ? bn(txParams.witnessLimit)
      : request.witnessLimit;
