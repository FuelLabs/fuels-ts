...
function getInstructions(
  ...
  for (let i = 0; i < offsets.length; i += 1) {
    multiCallInstructions.extend(
      getSingleCallInstructions(offsets[i], outputs[i]).entries(),
    );
  }
...
const getMainCallReceipt = (
  ...
  receipts.find(
    ({ type, from, to }) =>
      ...
      from === SCRIPT_WRAPPER_CONTRACT_ID &&
      to === contractId,
  );

const scriptResultDecoder =
  (contractId: AbstractAddress, isOutputDataHeap: boolean) =>
  (result: ScriptResult) => {
    ...
    return receipts
      ...
      .flatMap((receipt: ReturnReceipt, index, filtered) => {
        ...
        if (receipt.type === ReceiptType.Return) {
          return [
            new BigNumberCoder('u64').encode(
              (receipt as TransactionResultReturnReceipt).val,
            ),
          ];
        }
...
export const decodeContractCallScriptResult = (
  ...
  decodeCallResult(
    callResult,
    scriptResultDecoder(contractId, isOutputDataHeap),
    logs,
  );
...
export const getScriptDataV0: ContractCallScriptFn = (
  ...
  // 2. Asset ID
  scriptData.push(
    new B256Coder().encode(call.assetId?.toString() || ZeroBytes32),
  );
  // 3. Contract ID
  ...
  if (call.isInputDataPointer) {
    const pointerInputOffset =
      segmentOffset + POINTER_DATA_OFFSET + gasForwardedSize;
...
export const getScriptDataV1: ContractCallScriptFn = (
  ...
  const encodedSelectorOffset =
    callDataOffset + CONTRACT_ID_LEN + WORD_SIZE + WORD_SIZE;
  const encodedArgsOffset =
    encodedSelectorOffset + call.fnSelectorBytes.byteLength;
 ...
 */
export const getScriptDataForEncoding = (
  encoding?: string,
): ContractCallScriptFn => {
...
export const getContractCallScript = (
  ...
  new ScriptRequest<ContractCall[], Uint8Array[]>(
    ...
    (contractCalls): EncodedScriptCall => {
      const totalCalls = contractCalls.length;
      ...
      // Base offset
      const dataOffset =
        calculateScriptDataBaseOffset(maxInputs.toNumber()) +
        paddedInstructionsLength;
      ...
      for (let i = 0; i < totalCalls; i += 1) {
        ...
        const { scriptData: callScriptData, callParamOffsets } =
          getScriptDataForEncoding(call.encoding)(call, segmentOffset);
