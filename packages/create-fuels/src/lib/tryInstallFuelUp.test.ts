...
const mockAllDeps = (params: {
	isFuelUpInstalled?: boolean;
	shouldInstallFuelUp?: boolean;
}) => {
...
describe("tryInstallFuelup", () => {
	...
	it("should prompt install if fuelup not installed", async () => {
		// Arrange
		const { oraInstance, checkIfFuelUpInstalled, promptFuelUpInstall } =
			mockAllDeps({
				isFuelUpInstalled: false,
			});
	...
	it("should warn the user to install manually", async () => {
		...
		expect(warn.mock.calls[0][0]).toMatch(
			/You will need to install fuelup manually./g,
		);
