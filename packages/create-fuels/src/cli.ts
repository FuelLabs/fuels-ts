...
const processWorkspaceToml = (
  ...
  parsed.workspace.members = parsed.workspace.members.filter(
    (m) => programsToInclude[m],
  );
...
function writeEnvFile(
  ...
  let newFileContents = Object.entries(programsToInclude)
    .map(
      ([program, include]) =>
        `NEXT_PUBLIC_HAS_${program.toUpperCase()}=${include}`,
    )
    .join('\n');
...
export const runScaffoldCli = async ({
  ...
  const cliChosenPackageManager = Object.entries(cliPackageManagerChoices).find(
    ([, v]) => v,
  )?.[0];

  let packageManager =
    cliChosenPackageManager ?? (await promptForPackageManager());
  ...
  const hasAnyCliProgramsToInclude = Object.values(cliProgramsToInclude).some(
    (v) => v,
  );
  ...
  while (
    !programsToInclude.contract &&
    !programsToInclude.predicate &&
    !programsToInclude.script
  ) {
  ...
  const newForcTomlContents = processWorkspaceToml(
    forcTomlContents,
    programsToInclude,
  );
  ...
  if (shouldInstallDeps) {
    ...
    execSync(`${packageManager} install`, {
      stdio: verboseEnabled ? 'inherit' : 'pipe',
    });
  }
  ...
  log();
  log(
    chalk.green(`⚡️ Success! Created a fullstack Fuel dapp at ${projectPath}`),
  );
  log();
  log();
  log('To get started:');
  ...
  log(
    `- Run the frontend: ${packageManager} ${
      packageManager === 'npm' ? 'run ' : ''
    }dev`,
  );
  log();
