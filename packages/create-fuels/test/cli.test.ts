...
const filterOriginalTemplateFiles = (
	files: string[],
	programsToInclude: ProgramsToInclude,
) => {
	let newFiles = [...files];
...
describe("CLI", () => {
	test.each(possibleProgramsToInclude)(
		...
		async (programsToInclude) => {
			...
			let originalTemplateFiles = await getAllFiles(
				join(__dirname, "../templates/nextjs"),
			);
			originalTemplateFiles = filterOriginalTemplateFiles(
