...
// This whole file is copied and adapted from @ethersproject/abi 5.6.4
// "ethersproject/abi/src.ts/fragments.ts"
// The reason is that Fuel/Eth have some differences in the ABI
...
export class ParamType {
  ...
  constructor(constructorGuard: any, params: any) {
    ...
    if (match) {
      ...
      if (this.type === '[b256; 3]') {
        ...
        console.log(this.typeArguments);
      }
      populate(this, {
        arrayLength: null,
        arrayChildren: null,
  ...
  format(format?: string): string {
    ...
    if (format === FormatTypes.json) {
      ...
      if (this.typeArguments) {
        result.typeArguments = this.typeArguments.map((comp) => JSON.parse(comp.format(format)));
      }
    ...
    if (this.baseType === 'array') {
      ...
      if (this.typeArguments) {
        result += `<${this.typeArguments
          .map((typeArg) => typeArg.format(format))
  ...
  static fromObject(value: JsonFragmentType | ParamType): ParamType {
    ...
    return new ParamType(_constructorGuard, {
      ...
      typeArguments: value.typeArguments ? value.typeArguments.map(ParamType.fromObject) : null,
    });
  }
