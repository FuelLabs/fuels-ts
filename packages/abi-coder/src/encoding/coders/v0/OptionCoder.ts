...
import type { DecodedValueOf, InputValueOf } from './EnumCoder';
import { EnumCoder } from './EnumCoder';

type SwayOption<T> = { None: [] } | { Some: T };
export type Option<T> = T | undefined;

export class OptionCoder<
  TCoders extends Record<string, Coder>,
> extends EnumCoder<TCoders> {
  encode(value: InputValueOf<TCoders>): Uint8Array {
    const result = super.encode(
      this.toSwayOption(value) as unknown as InputValueOf<TCoders>,
    );
    return result;
  }
