...
export class U256Coder extends Coder<BNInput, BN> {
  constructor() {
    super('u256', 'u256', WORD_SIZE * 4);
  }
  ...
  encode(value: BNInput): Uint8Array {
    ...
    try {
      bytes = toBytes(value, WORD_SIZE * 4);
    } catch (error) {
  ...
  decode(data: Uint8Array, offset: number): [BN, number] {
    ...
    let bytes = data.slice(offset, offset + WORD_SIZE * 4);
    bytes = bytes.slice(0, WORD_SIZE * 4);

    if (bytes.length !== this.encodedLength) {
      throw new FuelError(ErrorCode.DECODE_ERROR, `Invalid ${this.type} byte data size.`);
    }

    return [bn(bytes), offset + WORD_SIZE * 4];
  }
}
