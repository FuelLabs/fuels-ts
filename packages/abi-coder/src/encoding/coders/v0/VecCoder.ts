...
type DecodedValueOf<TCoder extends Coder> = Array<
  TypesOfCoder<TCoder>['Decoded']
>;
...
> {
  ...
  decode(data: Uint8Array, offset: number): [DecodedValueOf<TCoder>, number] {
    ...
    const encodedLength = bn(
      new BigNumberCoder('u64').decode(len, 0)[0],
    ).toNumber();
    const vectorRawDataLength = encodedLength * this.coder.encodedLength;
    const vectorRawData = data.slice(
      BASE_VECTOR_OFFSET,
      BASE_VECTOR_OFFSET + vectorRawDataLength,
    );
