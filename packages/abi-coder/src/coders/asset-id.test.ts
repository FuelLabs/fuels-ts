...
describe('AssetIdCoder', () => {
  ...
  it('should throw an error when encoding a 256 bit hash string that is too short', async () => {
    ...
    await expectToThrowFuelError(
      ...
      new FuelError(ErrorCode.ENCODE_ERROR, 'Invalid struct AssetId.')
    );
  });
  ...
  it('should throw an error when decoding an encoded 256 bit hash string that is too short', async () => {
    ...
    await expectToThrowFuelError(
      ...
      new FuelError(ErrorCode.DECODE_ERROR, 'Invalid AssetId data size.')
    );
  });
  ...
  it('should throw an error when decoding an encoded 256 bit hash string that is too long', async () => {
    const invalidInput = new Uint8Array(Array.from(Array(32).keys()));

    await expectToThrowFuelError(
      () => coder.decode(invalidInput, 1),
      new FuelError(ErrorCode.DECODE_ERROR, 'Invalid AssetId byte data size.')
    );
  });
