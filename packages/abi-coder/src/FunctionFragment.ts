...
> {
	...
	private static getFunctionSelector(functionSignature: string) {
		const hashedFunctionSignature = sha256(
			bufferFromString(functionSignature, "utf-8"),
		);
	...
	#isInputDataPointer(): boolean {
		const inputTypes = this.jsonFn.inputs.map((i) =>
			findTypeById(this.jsonAbi, i.type),
		);

		return (
			this.jsonFn.inputs.length > 1 || isPointerType(inputTypes[0]?.type || "")
		);
	}
	...
	encodeArguments(values: InputValue[], offset = 0): Uint8Array {
		FunctionFragment.verifyArgsAndInputsAlign(
			values,
			this.jsonFn.inputs,
			this.jsonAbi,
		);
		...
		const results: Uint8ArrayWithDynamicData = new TupleCoder(coders).encode(
			shallowCopyValues,
		);
	...
	private static verifyArgsAndInputsAlign(
		...
		const errorMsg = `Mismatch between provided arguments and expected ABI inputs. Provided ${
			...
			inputs.length - optionalInputs.length
		} (excluding ${optionalInputs.length} optional inputs).`;
