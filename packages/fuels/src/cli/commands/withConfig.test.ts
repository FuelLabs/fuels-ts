...
describe("withConfig", () => {
	...
	function mockAll(params?: {
		shouldErrorOnDeploy?: boolean;
		shouldErrorOnLoadConfig?: boolean;
	}) {
	...
	test("onSuccess hook in config file", async () => {
		const { command, deploy, configPath, loadConfig, onSuccess, onFailure } =
			mockAll({
	...
	test("onFailure hook in config file", async () => {
		...
		expect(onFailure.mock.calls[0][0].toString()).toMatch(
			/something.+happened/i,
		);
	});

	test("should handle error when loading config file", async () => {
		const { command, deploy, error, loadConfig, configPath, onSuccess } =
			mockAll({
