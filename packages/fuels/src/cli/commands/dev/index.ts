...
export const workspaceFileChanged =
  (state: DevState) => async (_event: string, path: string) => {
    log(`\nFile changed: ${path}`);
...
export const dev = async (config: FuelsConfig) => {
  ...
  try {
    ...
    watchHandlers.push(
      watch(configFilePaths, options).on('all', configFileChanged(state)),
    );

    // watch: Forc's workspace members
    watchHandlers.push(
      watch(workspaceFilePaths, options).on('all', workspaceFileChanged(state)),
    );
  } catch (err: unknown) {
    error(err);
    throw err;
