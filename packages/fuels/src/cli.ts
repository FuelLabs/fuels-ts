...
export const configureCli = () => {
	...
	(command = program.command(Commands.init))
		...
		.addOption(
			new Option(`-c, --contracts ${arg}`, `${desc} Contracts`).conflicts(
				"workspace",
			),
		)
		.addOption(
		...
		.addOption(
			new Option(`-p, --predicates ${arg}`, `${desc} Predicates`).conflicts(
				"workspace",
			),
		)
		.requiredOption(
		...
		.option(
			...
			"Use buit-in `fuel-core` when starting a Fuel node",
		)
		.option(
			"--auto-start-fuel-core",
	...
	(command = program.command(Commands.dev))
		.description(
			"Start a Fuel node and run build + deploy on every file change",
		)
		.addOption(pathOption)
