...
describe('pbkdf2 node', () => {
  it('should use the registered function for PBKDF2 computation', () => {
    ...
    const passwordBuffer = bufferFromString(
      String('password123').normalize('NFKC'),
      'utf-8',
    );
    const saltBuffer = bufferFromString(
      String('salt456').normalize('NFKC'),
      'utf-8',
    );
    ...
    expect(pbkdf2(passwordBuffer, saltBuffer, iterations, keylen, algo)).toBe(
      expectedResult,
    );
  });
