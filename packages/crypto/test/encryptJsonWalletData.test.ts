import { decryptJsonWalletData, encryptJsonWalletData, randomBytes } from '..';
/**
 * @group node
 * @group browser
 */
describe('encryptJsonWalletData', () => {
  it('should encrypt and decrypt json wallet data correctly in a node environment', async () => {
    ...
    const encryptedData = await encryptJsonWalletData(
      testData,
      testKey,
      testIv,
    );
    ...
    const decryptedData = await decryptJsonWalletData(
      encryptedData,
      testKey,
      testIv,
    );
