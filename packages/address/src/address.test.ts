...
const ADDRESS_WORDS = [
  29, 30, 3, 10, 31, 10, 11, 9, 13, 19, 24, 13, 24, 24, 28, 5, 28, 11, 2, 0, 15,
  9, 23, 1, 11, 6, 16, 17, 0, 15, 7, 15, 22, 31, 17, 10, 28, 1, 17, 22, 31, 12,
  25, 29, 7, 18, 25, 10, 19, 25, 5, 0,
];
const ADDRESS_BYTES = [
...
describe('Address utils', () => {
  ...
  test('normalizeBech32 (bech32 to lowercase bech32)', () => {
    const result = utils.normalizeBech32(
      ADDRESS_BECH32.toUpperCase() as Bech32Address,
    );
  ...
  test('isPublicKey (too short)', () => {
    const result = utils.isPublicKey(
      '0xef86afa9696cf0dc6385e2c407a63d3cb2a9e4a',
    );
  ...
  test('clearFirst12BytesFromB256 (invalid B256)', async () => {
    ...
    await expectToThrowFuelError(
      () => utils.clearFirst12BytesFromB256(invalidB256),
      expectedError,
    );
  });
...
describe('Address class', () => {
  test('instantiate an Address class', () => {
    ...
    expect(`cast as string${result}`).toEqual(
      `cast as string${ADDRESS_BECH32}`,
    );
  ...
  test('create an Address class fromDynamicInput [bad input]', async () => {
    ...
    await expectToThrowFuelError(
      () => Address.fromDynamicInput('badinput'),
      expectedError,
    );
  });
  ...
  test('create an Address class using invalid Evm Address (no hex prefix)', async () => {
    ...
    await expectToThrowFuelError(
      () => Address.fromEvmAddress(address),
      expectedError,
    );
  });
});
