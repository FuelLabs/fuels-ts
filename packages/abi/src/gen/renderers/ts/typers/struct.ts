...
import type { TyperReturn, Typer, GlobalTyper } from './types';
...
export function mapComponents(params: {
  parent: AbiType | AbiTypeMetadata;
...
function mapGenericTypeParameters(
  typeArgs: AbiTypeMetadata['typeParameters'] | NonNullable<AbiType['metadata']>['typeArguments'],
  typer: GlobalTyper
): TyperReturn {
  if (!typeArgs) {
    return {
...
function getTypeNames(abiType: AbiType | AbiTypeMetadata) {
  const typeName =
...
function mapStructAsReference(abiType: AbiType | AbiTypeMetadata, typer: GlobalTyper): TyperReturn {
...
export const structTyper: Typer = ({ abiType, asReference }, typer) => {
  if ('concreteTypeId' in abiType || asReference) {
    return mapStructAsReference(abiType, typer);
  }

  const { inputName, outputName } = getTypeNames(abiType);

  const typeParameters = mapGenericTypeParameters(abiType.typeParameters, typer);
