...
const getDiff = () => {
  const base = JSON.parse(readFileSync('coverage/report/coverage-summary.json').toString());
...
(() => {
  ...
  ***Coverage Report (NO BASE):***
  ...
  writeFileSync('coverage/report/coverage-diff-no-base.txt', report, { flag: 'w' });
})();
